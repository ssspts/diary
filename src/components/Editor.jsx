...
function EditorInner({ selectedDiary, selectedDate, pages, setPages, currentPage, setCurrentPage, isDirty, setIsDirty, hasEdits, setHasEdits, saving, entryTitle, setEntryTitle, onSave, onOpenTemplatePicker }) {
  ...
  // Helper: marks both dirty (save button) AND hasEdits (navigation warning)
  // Call this whenever the user actually types/changes something
  const markEdited = useCallback(() => {
    setIsDirty(true);
    setHasEdits(true);
  }, [setIsDirty, setHasEdits]);
  ...
  const handlePageChange = useCallback((pageIdx, newText) => {
    ...
    if (countWrappedLines(newText, fontFamily) <= MAX_LINES_PER_PAGE) {
      ...
      setPages(updated); markEdited(); return;
    }
  ...
  const deletePage = (idx) => {
    ...
    setPages(pages.filter((_,i)=>i!==idx).map(ensureMeta)); markEdited();
  };
